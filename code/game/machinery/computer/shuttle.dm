/obj/machinery/computer/shuttle
	...
	attackby(var/obj/item/card/W as obj, var/mob/user as mob)
		...
		if (istype(W, /obj/item/card/id)||istype(W, /obj/item/modular_computer))
			...
			if(SSevac.evacuation_controller.is_prepared() && user.get_active_held_item() != W)
				return 0
			switch(choice)
				if("Authorize")
			...
			if(!emagged && !SSevac.evacuation_controller.is_prepared() && user.get_active_held_item() == W)
				switch(choice)
					if("Launch")
						to_world("<span class='notice'><b>Alert: Shuttle launch time shortened to 10 seconds!</b></span>")
						SSevac.evacuation_controller.set_launch_time(world.time+100)
