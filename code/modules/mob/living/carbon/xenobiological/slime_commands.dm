...
/decl/slime_command/follow/get_response(var/speaker, var/spoken, var/datum/ai/slime/holder)
	if(holder.leader)
		...
		if(LAZYACCESS(holder.observed_friends, "\ref[speaker]") > LAZYACCESS(holder.observed_friends, "\ref[holder.leader]"))
			holder.leader = speaker
			return "Yes... I follow [speaker]..."
		return "No... I follow [holder.leader]..."
	if(LAZYACCESS(holder.observed_friends, "\ref[speaker]") > 2)
		holder.leader = speaker
		return "I follow..."
...
/decl/slime_command/stop/get_response(var/speaker, var/spoken, var/datum/ai/slime/holder)
	var/friendship = LAZYACCESS(holder.observed_friends, "\ref[speaker]")
	...
	if(holder.leader)
		...
		if(friendship > LAZYACCESS(holder.observed_friends, "\ref[holder.leader]"))
			holder.leader = null
			return "Yes... I'll stop..."
...
/decl/slime_command/stay/get_response(var/speaker, var/spoken, var/datum/ai/slime/holder)
	...
	if(holder.leader)
		if(holder.leader == speaker)
			...
			return "Yes... Staying..."
		var/leader_friendship = LAZYACCESS(holder.observed_friends, "\ref[holder.leader]")
